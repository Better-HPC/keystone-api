...
The batch API allows multiple API requests to be submitted together and executed atomically within a single transaction.
If any step in the batch job fails, all changes made by the job are rolled back, ensuring the database is never left in 
a partial state.
