...
class CreateMethod(TestCase):
    ...
    def test_slug_matches_team_name(self) -> None:
        ...
        team = serializer.create(serializer.validated_data)
        self.assertEqual(slugify("My Unique Team"), team.slug)

    def test_manual_slug_ignored(self) -> None:
        """Verify manually specified slugs are ignored."""

        serializer = TeamSerializer(data={"name": "My Unique Team", "slug": "wrong-slug"})
        self.assertTrue(serializer.is_valid(), serializer.errors)
...
class UpdateMethod(TestCase):
    ...
    def test_slug_updated_to_match_name(self) -> None:
        ...
        self.assertEqual("Completely New Team Name", updated_team.name)
